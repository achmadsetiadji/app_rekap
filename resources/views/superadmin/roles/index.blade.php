        ...
        <div class="row">
                    ...
                    @foreach ($usersByRoles as $role)
                        @if (request('name') == $role->name)
                            <a href="" class="text-decoration-none text-white text-capitalize">
                                <li class="border rounded mb-1 border-primary text-center py-2 font-weight-bold bg-primary">
                                    {{ $role->name }}</li>
                            </a>
                        @elseif (!request('name') && $role->name == 'superadmin')
                            <a href="{{ route('superadmin.role.index', ['name' => $role->name]) }}"
            ...
            <div class="col-lg-9">
                <x-card class="bg-f2f2f2">
                    <form class="row" action="{{ route('superadmin.role.set_permissions', $roleName) }}" method="post">
                        @csrf
                        @php
                            $index = 1;
                        @endphp
                    ...
                    @elseif ($index > 3) mt-3 @endif
                    ">
                                <x-card class="my-1">
                                    <h5 class="card-title text-capitalize">
                                        {{ $group }}<span class="badge badge-dark rounded-circle ml-2"
                                            style="padding: 2px 2px; line-height: 0"><i class="ti-help"></i></span>
                                    </h5>
                                    ...
                                    @foreach ($permissions->filter(fn($permission) => $permission->group == $group) as $key => $permission)
                                        <div class="form-group mb-0 d-flex justify-content-between align-items-center">
                                            <label for="customCheck{{ $key }}"
                                                class="text-capitalize">{{ trim(str_replace($group, '', $permission->name)) }}</label>
                                            <div class="custom-control custom-control-inline custom-checkbox mr-0">
                                                <input type="checkbox" class="custom-control-input"
                                                    id="customCheck{{ $key }}" name="permissions[]"
